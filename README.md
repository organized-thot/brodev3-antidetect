...
### Step 1: Baserow Setup

1. Sign up at [Baserow.io](https://baserow.io) or use your own self-hosted instance.
2. Create a new Database in your workspace.
3. Create a new Table (e.g., named "Profiles").
4. Add the following fields to your table (use "Single line text" type for all):
    - `name`: The name of the profile.
    - `select`: Used to select profiles in the console app (set to "X" to select).
    - `open`: Indicates if the profile is currently open.
    - `proxy`: Proxy configuration (e.g., `user:pass@ip:port`).
    - `proxyType`: The type of proxy (e.g., `http` or `socks5`).
    - `fingerprint`: Stores fingerprint data or status.


### Step 2: API Configuration

1. In Baserow, click on your account menu (top left) and go to **Settings > Database Tokens**.
2. Click **Create token +**.
3. Select the workspace where your database is located.
4. Give the token a name and ensure it has permissions to Create, Read, Update, and Delete rows in your table.
5. Copy the generated **Token**. You will need this for the `.env` file.
6. To get the **Table ID**:
    - Open your table in the browser.
    - The URL will look like `https://baserow.io/database/12345/table/54321/56789`.
