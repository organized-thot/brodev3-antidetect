...
async function check_Profile(name){
    ...
    try {
        const response = await baserowAPI.get(`/api/database/rows/table/${config.baserowTableId}/`, { params: { search: name } });
            ...
            if (rows[i].name == name) {
                return {
                    ...
                    save: async function() {
                        ...
                        await baserowAPI.patch(`/api/database/rows/table/${config.baserowTableId}/${this.id}/`, payload);
                    },
