...
async function check_Profile(name){
    ...
    try {
        const response = await baserowAPI.get(`/api/database/rows/table/${config.baserowTableId}/?user_field_names=true&search=${encodeURIComponent(name)}`);
...
async function update_Profile(name, data){
    ...
    try {
        ...
        if (check == false) {
            await baserowAPI.post(`/api/database/rows/table/${config.baserowTableId}/?user_field_names=true`, data);
        } else {
            const response = await baserowAPI.get(`/api/database/rows/table/${config.baserowTableId}/?user_field_names=true&search=${encodeURIComponent(name)}`);
            const rows = response.data.results;
            let rowId = null;
            ...
            if (rowId) {
                await baserowAPI.patch(`/api/database/rows/table/${config.baserowTableId}/${rowId}/?user_field_names=true`, data);
            }
        }
    } catch (error) {
        ...
        for (let i = 0; i < rows.length; i++){
            if (rows[i].name == name) {
                return {
                    ...
                    save: async function() {
                        await baserowAPI.patch(`/api/database/rows/table/${config.baserowTableId}/${this.id}/?user_field_names=true`, this.data);
                    },
...
let get_Profiles = async function(){
    await limiter.removeTokens(2);
    try {
        const response = await baserowAPI.get(`/api/database/rows/table/${config.baserowTableId}/?user_field_names=true`);
        let arr = [];
        const rows = response.data.results;
        rows.forEach(row => {
            arr.push(row.name);
